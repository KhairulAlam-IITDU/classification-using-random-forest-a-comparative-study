...
public class EntropyCalculation {
	...
	public Node maxGainedElement(List<List<Double>> feature, List<Double> label, int featureSubsetSize, Map<Double, Double> weights) {	
		...
		double parentGini = evaluateGini(decision, weights);	
		...
		for(int i = 0; i < selectedFeatures.size(); i++) {
			...
			Node n = evaluateGain(label, parentGini, singleFeature, weights);
	...
	private Node evaluateGain(List<Double> label, double parentGini, List<Double> featureColumn, Map<Double, Double> weights) {
		...
		for(int i = 0; i < featureColumn.size(); i++) {
			...
			l = evaluateGini(lessEqual, weights);
			g = evaluateGini(greater, weights);

			double avgChildGini = (((leq/(double)featureColumn.size())*l)+((gr/(double)featureColumn.size())*g));
			
			double Gain = parentGini - avgChildGini;
	...
	private  double evaluateGini(Map<Double, Integer> subsetData, Map<Double, Double> weights) {
		
		int distinctDecision = subsetData.size();
		
		double[] count = new double[distinctDecision];
		int n = 0;
		...
		for(Double key : keys){
			...
			count[n] = subsetData.get(key)*weights.get(key);
